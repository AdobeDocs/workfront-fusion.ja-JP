...
![変数return](assets/variable-value---2-350x55.png)
...
指定された文字列を昇順（アルファベット順）で並べ替えて返します。
...
指定された文字列を降順（アルファベット逆）で並べ替えて返します。

**構文：** `sortDescString(string1; string2; ...)`
...
各単語の最初の文字を大文字にし、スペースを削除することで、文字列をPascalCaseに変換します。
