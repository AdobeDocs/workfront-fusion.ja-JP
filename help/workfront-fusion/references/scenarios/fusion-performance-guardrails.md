...
例えば、シナリオは、タスクが作成されるとトリガーされ、そのシナリオによってタスクが作成されます。 新しく作成されたタスクは、シナリオを再度トリガーし、別のタスクが作成されます。 タスクが作成されるたびにシナリオがトリガーされ、シナリオが実行されるたびにタスクが作成されます。
...
* 再帰はシナリオの設計の結果なので、シナリオをトリガーにするアクションがシナリオの tjat に含まれないようにシナリオを設計する必要があります。
