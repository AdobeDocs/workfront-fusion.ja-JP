...
* Webhookのキューは、キューに入れたイベントを最大&#x200B;**100,000**&#x200B;個まで保持できます。 Webhookを使用するシナリオがオフになっている場合、またはスケジュールで即座に実行するように設定されている場合、キューは一杯になります。 キューが100,000件のイベントに達すると、新しいイベントは「キューがいっぱいです」エラーと400件のステータスコードで拒否されます。

  Workfrontおよびプランニングイベントの場合、キューが長時間完全のままの場合、イベントサブスクリプションは無効になり、その後フリーズされ、Workfront Fusionはそのサブスクリプションのイベントの受信を停止します。
