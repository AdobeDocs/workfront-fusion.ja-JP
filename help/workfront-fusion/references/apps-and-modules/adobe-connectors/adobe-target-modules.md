...
description: Adobe Workfront Fusion シナリオでは、 [!DNL Adobe Target], as well as connect it to multiple third-party applications and services. [!DNL Adobe Target]  モジュールを使用してレコードを作成、読み取り、更新、削除したり、特定のタイプのすべてのレコードを一覧表示したり、指定した条件に基づいてレコードを検索したり、 [!DNL Adobe Target] APIにカスタム API呼び出しを実行したりできるワークフローを自動化できます。
author: Becky
...
[!DNL Adobe Target] コネクタを使用する前に、以下の前提条件が満たされていることを確認してください。

* アクティブな [!DNL Adobe Target] アカウントが必要です。

## Adobe Target APIについて

Adobe Target コネクタでは、次の機能が使用されます。
   ...
   <table style="table-layout:auto"> 
    ...
    <tbody>
      ...
      <tr>
        ...
        <td>サービスアカウント接続とAdobe Target サーバー間接続のどちらを作成するかを選択します。<p><b>重要</b>: 2024年6月3日以降に作成された接続には、Adobe Target サーバー間の接続が必要です。 既存のサービスアカウントの接続は、2025年1月まで引き続き機能します。 2024年1月までに、サービスアカウントの接続をAdobe Target サーバー間の接続に置き換える必要があります。
        </td>
      </tr>
      <tr>
  ...
  <tr>
    ...
    <td>
      ...
      <ul>
        ...
        <li>
        <b> オファーのレコメンデーション </b><p>フィールドについて詳しくは、Adobe Target API ドキュメントの<a href="https://developer.adobe.com/target/administer/admin-api/#tag/Offers/operation/createOffer">新しいレコメンデーションオファーの作成</a>を参照してください。</p>
        </li>
        <li>
          <b>[!UICONTROL オファーJSON]</b>
        ...
        <li>
        <b>環境</b><p>フィールドについて詳しくは、Adobe Target API ドキュメントの<a href="https://developer.adobe.com/target/administer/admin-api/#tag/Environments/operation/createEnvironment">Create environment</a>を参照してください。</p>
        </li>
        <li>
...
このアクションモジュールは、Targetのレコードを更新します。
...
<table style="table-layout:auto"> 
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>更新するフィールドを選択します。 フィールドは下に表示されます。
