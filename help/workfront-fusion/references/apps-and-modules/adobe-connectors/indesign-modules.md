...
このモジュールは、PDF ドキュメントを編集可能なInDesign（INDDまたはIDML）形式に変換します。 出力は、変換されたドキュメントと関連するアセットを含む、各入力PDFにちなんで名前が付けられたサブフォルダーを含むZIP ファイル（デフォルト名「output.zip」）です。 「リンクを埋め込む」オプションがfalseの場合、アセットはZIP内の別のフォルダーに提供されます。 trueの場合、すべてのリンクはInDesign ファイルに埋め込まれます。
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td role="rowheader">入力アセット</td>
      <td>変換する各アセットについて、<b>項目を追加</b>をクリックし、アセットのURLを入力してローカルファイル名を割り当てます。 ファイル名は、後でモジュールで参照されます。</td>
    </tr>
  <tr> 
   <td>ターゲットドキュメント</td> 
   <td>変換する各ドキュメントについて、<b>項目を追加</b>をクリックし、「入力アセット」フィールドから割り当てられたファイル名を入力します。</td> 
  </tr> 
  <tr> 
   <td>出力形式</td> 
   <td>ファイルをINDDまたはIDML ファイルに変換するかどうかを選択します。</td> 
  </tr> 
  <tr> 
   <td>埋め込みリンク</td> 
   <td>すべての画像とアセットリンクをINDDまたはIDML ファイルに直接埋め込む場合は、「はい」を選択します。 「いいえ」を選択して、これらのアセットをZIP ファイル内の別のフォルダーに配置します。</td> 
  </tr> 
  <tr> 
   <td>出力ZIP ファイル名</td> 
   <td>出力ZIP ファイルの名前を入力するか、名前を付けます。</td> 
  </tr> 
  <tr> 
   <td>出力</td> 
   <td>出力する各ファイルについて、ストレージタイプを選択し、ストレージの詳細を入力します。</td> 
  </tr> 
  <tr> 
   <td>返される結果の最大数</td> 
   <td>実行サイクルごとにモジュールが返す結果の最大数を入力します。</td> 
  </tr> 
  </tbody>
</table>
...
このモジュールは、登録のためにカスタムスクリプトバンドルを送信し、登録されたスクリプトの実行要求を投稿するためのURLを返します。
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td role="rowheader">スクリプト バンドル</td>
      <td>ドキュメントモジュールのダウンロードなど、以前のモジュールからソースファイルをマッピングします。 これはZIP ファイルである必要があります。</td>
    </tr>
  <tr> 
   <td>ファイル名</td> 
   <td>スクリプトバンドルを含むアップロード済みファイルの名前を入力またはマッピングします。</td> 
  </tr> 
  </tbody>
</table>
...
このモジュールは、登録されたカスタムスクリプトのInDesign アプリのバージョン設定を更新します。 これにより、最新バージョンの使用、メジャーバージョンの修正、特定のメジャーバージョンとマイナーバージョンの修正など、バージョン戦略を指定できます。
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td role="rowheader">スクリプト名</td>
      <td>更新するカスタムスクリプトの名前を入力またはマッピングします。 これは、スクリプトの登録時に返される<code>capability</code>値です。</td>
    </tr>
  <tr> 
   <td>アプリのバージョン戦略</td> 
   <td>使用するアプリのバージョン戦略を選択します。
   <ul>
   <li><b>常に最新バージョンを使用する</b></li>
   <li><b>メジャーバージョンに固定</b><p>これを適用するメジャーバージョンの番号を入力またはマッピングします。</p></li>
   <li><b>メジャーバージョンとマイナーバージョンのピン留め</b><p>これを適用するメジャーバージョンとマイナーバージョンを入力します。</p></li>
   </ul></td> 
  </tr> 
  </tbody>
</table>
...
このモジュールは、登録された各アプリバージョンのメジャーバージョン、マイナーバージョン、ステータスなど、利用可能なすべてのInDesign アプリバージョンに関する情報を取得します。
