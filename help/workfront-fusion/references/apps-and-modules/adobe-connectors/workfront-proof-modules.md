...
これは、特定のイベントに基づいてプルーフを更新したり、プルーフの受信者を検索したりするなど、現在Workfrontまたは[!DNL Workfront Proof]のプルーフでサポートされていないタスクを実行する必要がある場合に便利です。
...
Workfront Proof コネクタでは、次の機能が使用されます。
...
Workfront Fusion モジュール内から直接[!DNL Workfront Proof] アカウントへの接続を作成できます。

1. Workfront Fusion モジュールで、[!UICONTROL Connection] フィールドの横にある&#x200B;[!UICONTROL **Add**]&#x200B;をクリックします
   ...
   <table style="table-layout:auto"> 
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>これが本番環境であるか、プレビューやサンドボックスなどの非本番環境であるかを選択します。</td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td>接続する実稼動環境、プレビュー環境またはカスタム環境。</td>
            </tr>
        </tbody>
    </table>
...
<table style="table-layout:auto">
 ...
 <tbody> 
  <tr> 
   ...
   <td>新しいWebhookの名前を入力またはマッピングします</td> 
  </tr> 
  <tr> 
   <td>[!DNL Connection]</td> 
   <td> <p>[!DNL Workfront Proof] アカウントをWorkfront Fusionに接続する手順については、<a href="/help/workfront-fusion/create-scenarios/connect-to-apps/connect-to-fusion-general.md" class="MCXref xref" data-mc-variable-override="">Adobe Workfront Fusionへの接続の作成 – 基本手順</a>を参照してください。</p> </td> 
  </tr> 
 </tbody> 
</table>
...
<table style="table-layout:auto">
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td>新しい決定（プルーフステータスの変更を含む）を視聴するか、プルーフステータス全体の変更のみを視聴するかを選択します。</td> 
  </tr> 
  <tr> 
...
<table style="table-layout:auto">
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td>新しいプルーフを監視するか、新しい全体的なプルーフ決定を監視するかを選択します。</td> 
  </tr> 
  <tr data-mc-conditions=""> 
  ...
  <tr data-mc-conditions=""> 
   <td role="rowheader">ページあたりのアイテム数</td> 
   <td> <p>結果をページネーションするには、結果の各ページに表示される返された結果の数を入力またはマッピングします。 この数値は100以下にする必要があります。</p> </td> 
  </tr> 
 </tbody> 
</table>
...
新しいバージョンを作成する場合は、新しいプルーフのパラメーターとソースプルーフを指定します。

モジュールは、新しいプルーフまたはプルーフバージョンのIDを返します。 この情報は、シナリオ内の後続のモジュールにマッピングできます。
...
このアクションモジュールは、[!DNL Workfront Proof] API に対して認証済みのカスタム呼び出しを実行します。 これにより、他の [!DNL Workfront Proof] モジュールでは不可能なデータフロー自動処理を作成できます。
...
モジュールは、プルーフの作成に使用されたソースファイルのコンテンツを返します。この情報は、シナリオ内の後続のモジュールにマッピングできます。
...
<table style="table-layout:auto">
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p>[!UICONTROL Proof Details] ページにあるプルーフの一意のIDを入力します。  </td> 
  </tr> 
 </tbody> 
</table>
...
<table style="table-layout:auto">
 ...
 <tbody> 
  ...
  <tr> 
   <td>[!UICONTROL ID]</td> 
   <td>このモジュールで読み取るレコードの一意の [!DNL Workfront Proof] ID を入力またはマッピングします。</td> 
  </tr> 
 </tbody> 
</table>
...
このモジュールは、プルーフを検索する場合に、プルーフの ID を返します。 または、受信者を検索している場合は、受信者のユーザーID、電子メール、名前、役職、電子メールエイリアスを返します。この情報は、シナリオ内の後続のモジュールにマッピングできます。
