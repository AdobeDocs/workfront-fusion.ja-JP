...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td> <p>配列項目を追加して、カスタムバンドルを作成します。 バンドルに追加する各アイテムについて、<b> アイテムを追加</b>をクリックし、アイテムの名前と値を入力します。</p> </td> 
  </tr> 
 </tbody> 
</table>
...
<table style="table-layout:auto">
    <tr>
        ...
        <td>モジュールを取得する各変数について、<b>項目を追加</b>をクリックし、変数の名前を入力します。</td>
    </tr>
</table>
...
このモジュールは、各サイクルまたは各シナリオ実行後に1ずつ増分された値を返します。
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td> <p>モジュールで値をリセットするタイミングを選択します。 これは、値を最初の値からやり直す場合です。</p> 
    <ul> 
...
このモジュールは、タスク、リード、メールなどの「ラウンドロビン」割り当てをグループ内のユーザーに実装するために使用できます。 アルゴリズムは、ある合理的な順序でグループから割り当て先を選択します。通常は、リストの上から下に移動します。 アルゴリズムがリストの最後に到達すると、次の割り当てをリストの一番上のユーザーに行い、リストの下方に向かって割り当てを続けて行います。

次のシナリオでは、奇数番号のシナリオを実行するたびに最初の受信者にメールを送信し、偶数番号のシナリオを実行するたびに 2 番目の受信者にメールを送信します。

![電子メールの例](/help/workfront-fusion/references/apps-and-modules/assets/example-email.png)
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  <tr> 
   <td>[!UICONTROL Variables]</td> 
   <td>追加する各変数について、<b>項目を追加</b>をクリックし、変数の名前と値を入力します。</td> 
  </tr> 
  <tr> 
   <td>[!UICONTROL Variable lifetime] </td> 
   <td> <p>変数の有効期間を選択します（同じ値を維持）。</p> 
    <ul> 
     <li><strong>[!UICONTROL One cycle]</strong>：変数は 1 サイクルの間有効です。 これは、1つのシナリオ実行で複数のwebhookが受信される場合に便利です。より多くのwebhookがより多くのサイクルを作成するためです。 </li> 
...
>[!TIP]
>
>フローを長時間一時停止する場合は、シナリオを2つのシナリオに分割することをお勧めします。
>
...
任意の値を文字列データタイプ（テキスト）に変換します。 これにより、バイナリデータなどのマッピング時のマッピングが容易になります。
   ...
   <td> <p>正規表現を使用するには、このオプションを有効にします。 モジュールは、完全一致ではなく、正規表現に基づいて大文字と小文字を判断します。</p> 
    <div> 
     ...
     <ul> 
      ...
      <li>正規表現を試したい方には、<a href="https://regex101.com/">正規表現 101</a> web サイトをお勧めします。 左パネルで ECMAScript（JavaScript）FLAVOR を選択します。</li> 
     </ul> 
    </div> </td> 
  </tr> 
  <tr> 
   <td>[!UICONTROL Cases] </td> 
   <td> 追加するケースごとに、<b>項目を追加</b>をクリックし、項目のパターンと出力を入力します。 <p>入力に、[!UICONTROL Pattern] フィールドに入力された値が含まれている場合、[!UICONTROL Output] フィールドに入力された値が返されます。</p> <p>入力が、[!UICONTROL Pattern] フィールドに設定した値のいずれにも一致しない場合、次のいずれかが発生します。</p> 
    <ul> 
