...
Adobe Workfront Fusion [!DNL Figma] モジュールを使用すると、コメント、ファイル、ファイルのバージョンまたはプロジェクトのリストを取得できます。 また、コメントを投稿したり、[!DNL Figma] API を呼び出したりすることもできます。
...
>[!IMPORTANT]
>
>* Figmaは、2025年1月に認証要件を変更しました。 `Figma`接続タイプは新しい要件を満たしています。 `Figma (Legacy)`接続タイプは非推奨（廃止予定）です。近い将来に削除されます。
>* リダイレクト URLを使用している場合は、`https://oauth.app.workfrontfusion.com/oauth/cb/figma3`を使用します。

Figma モジュールの接続を作成するには：

1. 任意のFigma モジュールで、接続ボックスの横にある&#x200B;**[!UICONTROL 追加]**&#x200B;をクリックします。
...
1. 次のフィールドに入力します。
   ...
   <table style="table-layout:auto"> 
      ...
      <tbody>
        ...
        <td>
          <p> レガシータグを使用せずに<code>Figma</code>を選択します。 </p><p>Figmaは、2025年1月に認証要件を変更しました。 <code>Figma</code>接続タイプは新しい要件を満たしています。 <code>Figma (Legacy)</code>接続タイプは非推奨（廃止予定）であり、今後削除される予定です。</p>
        </td>
        </tr>
        <tr>
        ...
        <td>[!UICONTROL Figma] [!UICONTROL Client ID]を入力します。</td>
        </tr>
        <tr>
        ...
        <tr>
        <td role="rowheader">カスタムスコープ</td>
        <td>この接続に必要なカスタムスコープを入力します。</td>
        </tr>
        <tr>
        <td role="rowheader">カスタム接続の検証URL</td>
        <td>接続が正常に作成されたことを確認するためのデフォルトのエンドポイントは次のとおりです。<code>https://api.figma.com/v1/me</code> カスタムスコープでこのURLがサポートされていない場合は、カスタム検証URLを指定してください。</td>
        </tr>
      </tbody>
    </table>
...
<table style="table-layout:auto"> 
  ...
  <tbody>
    <tr>
      ...
      <td> <p>[!DNL Figma] アカウントをWorkfront Fusionに接続する手順については、この記事の「<a href="#create-a-connection-to-figma" class="MCXref xref" data-mc-variable-override="">Figma</a>への接続の作成」を参照してください。</p>
      </td>
    </tr>
    <tr>
...
<table style="table-layout:auto"> 
  ...
  <tbody>
    ...
    <tr>
      <td role="rowheader">[!UICONTROL Team ID]</td>
      <td>チームコンポーネントまたはチームコンポーネントセットを取得する場合は、レコードまたはレコードが関連付けられているチームのIDを入力またはマッピングします。</td>
    </tr>
    <tr>
      <td role="rowheader">[!UICONTROL Page Size]</td>
      <td>チームコンポーネントまたはチームコンポーネントセットを取得する場合は、ページごとに返される数値または結果を入力またはマッピングします。 デフォルト：30。</td>
    </tr>
    <tr>
      <td role="rowheader">[!UICONTROL After]</td>
      <td>
        <p>チームコンポーネントまたはチームコンポーネントセットを取得する場合は、結果の取得を開始する前の結果番号を入力またはマッピングします。 これを [!UICONTROL Page Size] フィールドと組み合わせて、結果をページ分割することができます。</p>
    ...
    <tr>
      ...
      <td>
        <p>チームコンポーネントまたはチームコンポーネントセットを取得する場合は、結果の取得を開始する前の結果数を入力またはマッピングします。 これを [!UICONTROL Page Size] フィールドと組み合わせて、結果をページ分割することができます。</p>
        <p>この値はオブジェクト ID に対応していません。</p>
...
<table style="table-layout:auto"> 
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        <p><code>https://api.figma.com/v1/</code> からの相対パスを入力します。</p>
...
このトリガー モジュールは、[!DNL Figma] チーム スペース内の特定のチームに対して次のいずれかのイベントが発生した場合にシナリオを開始します。
    ...
    <tr>
      ...
      <td>
        ...
        <ol>
          ...
          <li>
            <p>Webhookをアクティブにするか一時停止するかを選択します。</p>
          </li>
          <li>
            <p>Webhookの説明を入力します。</p>
          </li>
          <li>
