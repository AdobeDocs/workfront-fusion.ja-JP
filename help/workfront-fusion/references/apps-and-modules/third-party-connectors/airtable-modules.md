...
Adobe Workfront Fusion用の[!DNL Airtable] コネクタを使用すると、[!DNL Airtable] アカウント内のイベントに基づいてシナリオを開始し、レコードの作成、アップロード、更新、レコードの検索、Airtable APIへのカスタム API呼び出しを行うことができます。
...
1. 個人アクセストークンまたはOAuth 2認証のどちらを使用しているかを選択します。
1. 接続に名前を入力します。
1. （条件付き）個人アクセストークンを使用する場合は、「詳細設定を表示」をクリックし、個人アクセストークンを入力します。
1. （条件付き） OAuth 2を使用している場合は、「詳細設定を表示」をクリックし、Airtable クライアント IDとクライアントシークレットを入力します。
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td> <p>レコード</p> </td> 
   <td> <p>新しいレコードの値を入力します。 使用可能なフィールドは、選択したテーブルに基づきます。</p> 
   <!--
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td> <p>レコード</p> </td> 
   <td> <p>新しいレコードの値を入力します。 使用可能なフィールドは、選択したテーブルによって異なります。</p> 
   <!--
...
1. 手順2で生成したWebhook URLを表示されたダイアログボックスに入力し、Webhook URLの直後に？record_id={record_id}を追加して、モジュールの出力にレコード IDを含め、「保存」をクリックします。 結果の URL は、例えば、次のようになります。
...
このアクションモジュールは、[!DNL Airtable] API に対して認証済みのカスタム呼び出しを実行します。 これにより、他の [!DNL Airtable] モジュールでは不可能なデータフロー自動処理を作成できます。
...
<table style="table-layout:auto">
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td><code>https://api.airtable.com/</code> からの相対パスを入力します。 例：<code>v0/{base}/{table}</code> </td> 
  </tr> 
  <tr> 
