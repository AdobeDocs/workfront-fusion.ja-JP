...
description: Adobe Workfront Fusion のシナリオでは、 [!DNL Split.io] を使用するワークフローを自動化したり、複数のサードパーティのアプリケーションやサービスに接続したりすることができます。
author: Becky
...
Adobe Workfront Fusion のシナリオでは、[!DNL Split.io] を使用するワークフローを自動化したり、複数のサードパーティのアプリケーションやサービスに接続したりすることができます。
...
## [!DNL Split.io] を Workfront Fusion に接続  {#connect-split-io-to-workfront-fusion}
...
[!DNL split.io] モジュールを設定すると、Workfront Fusion には以下のフィールドが表示されます。 これらとともに、アプリやサービスのアクセスレベルなどの要因に応じて、追加の「[!DNL split.io]」フィールドが表示される場合があります。 モジュール内の太字のタイトルは、必須フィールドを示します。
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td>タグを追加するオブジェクトの名前を入力するか、マッピングします。</td> 
  </tr> 
  <tr> 
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p>定義に追加する各処理について、<b>[!UICONTROL Add item]</b>をクリックし、処理とサイズを入力またはマッピングします。</p> </td> 
  </tr> 
 </tbody> 
</table>
...
このアクションモジュールは、[!DNL split.io] API に対して認証済みのカスタム呼び出しを実行します。 これにより、他の [!DNL split.io] モジュールでは不可能なデータフロー自動処理を作成できます。
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td><code>https://api.split.io/internal/api/v2/</code> からの相対パスを入力します。</td> 
  </tr> 
  <tr> 
