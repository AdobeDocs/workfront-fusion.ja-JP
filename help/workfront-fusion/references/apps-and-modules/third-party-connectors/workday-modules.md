...
Adobe Workfront Fusion のシナリオでは、[!DNL Workday] を使用するワークフローを自動化したり、複数のサードパーティのアプリケーションやサービスに接続したりすることができます。
...
<table style="table-layout:auto"> 
    ...
    <tbody>
        <tr>
            ...
            <td>[!DNL Workday] アカウントをWorkfront Fusionに接続する手順については、<a href="#Connect" class="MCXref xref" >Workfront Fusionに[!DNL Workday]を接続</a>するを参照してください。</td>
        </tr>
        <tr>
...
このアクションモジュールは、[!DNL Workday] API に対して認証済みのカスタム呼び出しを実行します。 これにより、他の [!DNL Workday] モジュールでは不可能なデータフロー自動処理を作成できます。
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td><code style="color: #ff1493;">https://&lt;tenantHostname>/api/&lt;serviceName>/&lt;version>/&lt;tenant></code> からの相対パスを入力します。</td> 
  </tr> 
  <tr> 
...
このアクションモジュールは、[!DNL Workday] 内の 1 つのレコードを更新します。
