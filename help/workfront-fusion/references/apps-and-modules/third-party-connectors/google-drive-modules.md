...
Adobe Workfront Fusion [!DNL Google Drive] モジュールを使用すると、[!DNL Google Drive]内のファイル、フォルダー、または共有ドライブを監視、検索、作成、更新、削除、管理できます。

Adobe Workfront Fusion シナリオでは、[!DNL Google Drive] アカウントを複数のサードパーティ製アプリケーションやサービスに接続できます。
...
このトリガーモジュールは、[!DNL Google Drive]内のファイルが追加または変更されたときに開始されます。
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td>1回のサイクルでWorkfront Fusionがダウンロードする結果の最大数（シナリオ実行ごとの繰り返し数）を設定します。</td> 
  </tr> 
 </tbody> 
</table>
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td>1回のサイクルでWorkfront Fusionが返すコメントの最大数（シナリオ実行ごとに繰り返される回数）を設定します。</td> 
  </tr> 
 </tbody> 
</table>
...
このアクションモジュールは、指定した場所にフォルダーを作成します。
...
このアクションモジュールは、指定されたIDを持つファイルを取得します。
...
このアクションモジュールは、Google ドライブ内のファイルの共有リンクを取得します。
...
この検索モジュールは、検索条件に基づいてファイルまたはフォルダーを検索します。
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td>1回の実行サイクルでWorkfront Fusionが返すファイルまたはフォルダーの最大数を設定します。</td> 
  </tr> 
  <tr> 
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td>ファイルを特定のフォルダーに移動する場合は、ファイルを移動するフォルダーを選択します。</td> 
  </tr> 
  <tr> 
   <td>[!UICONTROL File ID]</td> 
  ...
  <tr> 
   <td>[!UICONTROL ファイルを変換]</td> 
   <td>ファイルを対応するGoogle ファイル形式に変換するには、このオプションを有効にします。</td> 
  </tr> 
 </tbody> 
</table>
...
ファイルのアップロードまたは更新が失敗する理由はいくつかあります。

* アップロードされたファイルが大きすぎるため、[!DNL Google Drive] プランで許可されている最大ファイルサイズの制限を超えているか、[!DNL Google Drive] ストレージの制限を超えています。 ストレージプランをアップグレードするか、[!DNL Google Drive] サービスから既存のファイルを削除することができます。
* ファイルのアップロード先として選択したフォルダーが存在しません。この場合、シナリオは停止し、モジュール内の別のターゲットフォルダーを選択する必要があります。
