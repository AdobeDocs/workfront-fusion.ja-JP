...
詳しくは、Workfront モジュールの「[&#x200B; イベントを見る](/help/workfront-fusion/references/apps-and-modules/adobe-connectors/workfront-modules.md#triggers)」を参照してください。

## ロックされたシナリオの管理

場合によっては、パフォーマンスやその他の問題が発生しているために、Workfront Fusion エンジニアリングチームによってシナリオが一時的にロックされる場合があります。 今回、ロックされたシナリオを Fusion 内からロック解除できるようになりました。
...
Fusion で操作を再試行したいだけの場合、一時停止ディレクティブは最適な選択肢でない可能性があります。 その場合は、リピーターモジュールを使用して再試行を設計することをお勧めします。 リピータ モジュールを使用した再試行の設計手順については、「[&#x200B; リピータ モジュールを使用する](/help/workfront-fusion/create-scenarios/config-error-handling/retry.md#use-the-repeater-module)」の「再試行エラーの処理」を参照してください。
