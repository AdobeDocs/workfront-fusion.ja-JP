...
* **時間の延長**: ファイルの書き出しや読み込みなどの長時間実行アクションを伴う大規模なバッチ処理には、チェーンを使用できます。 ファイルが多い場合、この操作には時間がかかります。 子シナリオは親シナリオのタイムアウトに対してカウントされないので、複数の子シナリオを使用してファイルを書き出したり読み込んだりすることで、実行時間を超えることができます。
